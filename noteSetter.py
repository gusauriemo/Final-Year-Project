...
def chord_notes(fileName, decisionValue):
    notes1=[]
    samplingFrequency, signalData = wav.read(fileName)
    #plot.figure()
    graph = plot.magnitude_spectrum(signalData,Fs=samplingFrequency)
    xvalues = graph[0]
    yvalues = graph[1] # this is switched -> y values are actually stored in the xvalues variable
    idx = np.where(xvalues>decisionValue)
    frequencies = yvalues[idx] #that is why the frequencies are being obtained from yvalues
    #here the frquencies up until 4000 and smth Hz need to be "binned"; once the index of these frequecies is related to a bin, then we can sum the magnitude of these frequencies!!
    #plot.show()
    #print(frequencies)
    for i in range(88) :
        out = np.where((frequencies/set_frequencies[i])<((2**(0.5/12))), frequencies, 0) #+ half note tolernce for bin
        out2= np.where((out/set_frequencies[i])>(2-(2**(0.5/12))), set_frequencies[i], 0) #- half note tolerance for bin
        for b in range(len(frequencies)):
            if out2[b]!=0:
                notes1.append(out2[b])
    sorted_notes = sorted(set(notes1))
    return sorted_notes
